...
def compute_average_scores(tag, classifier, x, y, label):
    ...
    print("True positive rate (tpr)  = {:.6f} [among all truth, model predicts as truth correctly]".format(tpr))
    ...
    w_recall = tp / (tp+fn)
    w_precision = tp / (tp+fp)
    w_f1 = 2.0 * precision * recall / (precision + recall)
...
def run_random_forest_classifier(features, label):
    ...
    compute_average_scores("Random Forest Train", classifier, train_x, train_y, label)
    compute_average_scores("Random Forest Test", classifier, test_x, test_y, label)
    return
